...
function errorHandler(ctx, error, query) {
  let type = '';
  if (oracledb.thin) {
    type = error.code.includes('ORA') ? 'database side ' : 'oracledb side ';
  }

  ctx.logger.error(`sqlQuery() ${type}error while executing query: ${query}: ${error.stack}`);
}
...
async function executeQuery(ctx, sqlCommand, values = [], noModifyRes = false, noLog = false) {
  ...
  try {
    ...
    if (!noLog) {
      errorHandler(ctx, error, sqlCommand);
    }
